...
- (void)viewDidLoad {
    [super viewDidLoad];
    self.appointments = [NSMutableDictionary new]; //Create new appointments dictionary.
...
- (void)updateCalenderWithDate:(NSDate *)date {
    self.selectedDate = date; //Updates the current date being viewed.
    [self.lblDate setStringValue: [self.selectedDate stringFromDateWithFormat: DateFormat_E_MMMd_yyyy]]; //Sets lblDate to the selected date.
    [self updateDailyViewWithArray:[self.appointments objectForKey:[self.selectedDate stringFromDateWithFormat: DateFormat_MM_dd_yyyy]]];
}
...
-(IBAction)adjustDateButtons_Click:(id)sender {
    ...
    [self updateCalenderWithDate:[[NSDate alloc] initWithHourInterval:hours sinceDate:self.selectedDate]]; //Sets the current date.
}

- (IBAction)showButton_Click:(id)sender {
    NSDateComponents *comp = [self.selectedDate getComponents]; //Used to create a new date with the components the user entered.
...
- (IBAction)createButton_Click:(id)sender {
    ...
    Appointment *appoint = [[Appointment alloc] initWithName:[self.txtDescription.string copy] atDate:appointTime]; //Appointment based on data entered by user.
    NSMutableArray *apptsArr; //Mutable Array to store all appointments in the given day.
    if ((apptsArr = [self.appointments objectForKey:key])) { //Checking to see if an array for the current day already exists in the Appointments dictionary.
    ...
    [self.appointments setObject:apptsArr forKey:key]; //Add the array to the dictionary.
...
- (IBAction)cancelButton_Click:(id)sender {
    ...
    NSMutableArray *apptsArr = [self.appointments objectForKey:key];
    ...
    [self.appointments setObject:apptsArr forKey:key]; //Update the dictionary.
...
-(void)setUpAppointmentWithTime:(NSDate **)time andKey:(NSString **)key {
    NSDateComponents *comp = [self.selectedDate getComponents];
    ...
    *key = [self.selectedDate stringFromDateWithFormat: DateFormat_MM_dd_yyyy];
}

-(void)updateDailyViewWithArray:(NSMutableArray *)array {
    NSMutableString *text = [NSMutableString new];
    for (Appointment *a in array) {
        [text appendString:[NSString stringWithFormat:@"%@ %@\n\n", [a.date stringFromDateWithFormat: DateFormat_H_mm], a.name]];
    }
